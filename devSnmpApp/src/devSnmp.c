...
static long init_mbbid_snmp(struct mbbiDirectRecord *pmbbid, long snmpVersion);
static long init_mbbid_snmpV1(struct mbbiDirectRecord *pmbbid);
static long init_mbbid_snmpV2c(struct mbbiDirectRecord *pmbbid);
static long read_mbbid_snmp(struct mbbiDirectRecord *pmbbid);
...
SNMP_DEV_SUP_SET devMbbiDSnmpV1 = {6, NULL, NULL, init_mbbid_snmpV1,  NULL, read_mbbid_snmp, NULL};
...
SNMP_DEV_SUP_SET devMbbiDSnmpV2c = {6, NULL, NULL, init_mbbid_snmpV2c,  NULL, read_mbbid_snmp, NULL};
...
epicsExportAddress(dset, devMbbiDSnmpV1);
...
epicsExportAddress(dset, devMbbiDSnmpV2c);
#endif
...
static long init_mbbid_snmp(struct mbbiDirectRecord *pmbbid, long snmpVersion)
{
    ...
    /* mbbid.inp must be INST_IO */
    if(pmbbid->inp.type != INST_IO)
    {
        recGblRecordError(S_db_badField,(void *)pmbbid, "devMbbiDSnmp (init_record) Illegal INP field");
        pmbbid->pact = TRUE;
        return(S_db_badField);
    }

    status = snmpRequestInit((dbCommon *)pmbbid, pmbbid->inp.value.instio.string, snmpVersion, MAX_CA_STRING_SIZE, 0, 'b');

    if (status)
    {
        recGblRecordError(S_db_badField, (void *)pmbbid,"devMbbiDSnmp (init_record) bad parameters");
        pmbbid->pact = TRUE;
        return(S_db_badField);
    }
...
static long init_mbbid_snmpV1(struct mbbiDirectRecord *pmbbid)
{
    return init_mbbid_snmp(pmbbid, SNMP_VERSION_1);
}

static long init_mbbid_snmpV2c(struct mbbiDirectRecord *pmbbid)
{
    return init_mbbid_snmp(pmbbid, SNMP_VERSION_2c);
}

static long read_mbbid_snmp(struct mbbiDirectRecord *pmbbid)
{
    SNMP_REQUEST  *pRequest = (SNMP_REQUEST *)(pmbbid->dpvt);
    int rtn = -1;
    char *pValStr;
    unsigned int u32temp;
    ...
    {/* pre-process */
        ...
        if(epicsMessageQueueTrySend(pRequest->pSnmpAgent->msgQ_id, (void *)&pRequest, sizeof(SNMP_REQUEST *)) == -1)
        {
            ...
            errlogPrintf("Send Message to Snmp Operation Thread Error [%s]", pmbbid->name);
            rtn = -1;
        }
        else
        {
    ...
    {/* post-process */
        ...
        else
        {
            ...
            /* skip non-digit, particularly because of WIENER crate has ON(1), OFF(0) */
            for (; isdigit(*pValStr) == 0 && *pValStr != '\0'; ) ++pValStr;
            /* The mbbiDirect record for snmp only handles unsigned int */
            if (pValStr && sscanf(pValStr, "%u", &u32temp))
            {
                pmbbid->val = u32temp&0x7FFFFFFF;	/* Get rid of MSB since pmbbid->val is signed */
                pmbbid->udf = FALSE;
                rtn = NO_CONVERT;
            }
            else
